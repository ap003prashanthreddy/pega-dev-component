...
################################################################################
# GenAI Component Catalog Release Wrapper
# Usage: ./genai_release.sh [--dry-run] [--no-commit] <version> <work_item> <release_date>
# Example: ./genai_release.sh 0.0.18 RLS-00000 2026-05-26
# Dry-run: ./genai_release.sh --dry-run 0.0.18 RLS-00000 2026-05-26
# No-commit: ./genai_release.sh --no-commit 0.0.18 RLS-00000 2026-05-26
################################################################################
...
MANIFEST_FILE="${REPO_ROOT}/manifests/genai.json"
...
if [ "$#" -ne 3 ]; then
    ...
    echo ""
    echo "Arguments:"
    echo "  version       - Semantic version (e.g., 0.0.18)"
    echo "  work_item     - Work item ID (e.g., RLS-00000)"
    echo "  release_date  - Release date in YYYY-MM-DD format (e.g., 2026-05-26)"
    echo ""
    echo "Options:"
    ...
    echo ""
    echo "Examples:"
    echo "  $0 0.0.18 RLS-00000 2026-05-26"
    echo "  $0 --dry-run 0.0.18 RLS-00000 2026-05-26"
    echo "  $0 --no-commit 0.0.18 RLS-00000 2026-05-26"
    echo ""
    exit 1
fi
...
echo "GenAI Component Catalog Release"
echo "================================"
if [ $DRY_RUN -eq 1 ]; then
    echo "[DRY-RUN MODE]"
    echo ""
